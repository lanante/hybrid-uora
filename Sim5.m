...
for a=1:length(N_STA_Array)
    ...
    k=kA(1:U);
    tpdf=tpdfA(1:U);
    [e1_Tput(a) e1_Efficiency(a) ready_tau e1_p e1_b_idle e1_B e1_Latency(a) nTF tTF]=randomaccess_nfb10(EOCWminS(a),EOCWmaxS(a),n,r,200,U,5,1,tpdf,k);
    ...
    for c=1:length(EOCWmaxArray)
        for d=c:length(EOCWminArray)
            e=e+1;
                ...
                [ea_Tput(a,e) ea_Efficiency(a,e) ready_tau e1_p e1_b_idle e1_B ea_Latency(a,e) nTF tTF]=randomaccess_nfb10(EOCWminArray(d),EOCWmaxArray(c),n,r,200,U,5,1,tpdf,k);
...
figure(2)
plot(N_STA_Array,e1_Latency/1e-3','k-o','LineWidth',2);hold on
plot(N_STA_Array,ea_Latency/1e-3,'k*');
legend('H-UORA with U=7','Random')
axis([15 105 0 0.07/1e-3])
grid
ylabel('Latency (ms)')
xlabel('Number of STAs')
